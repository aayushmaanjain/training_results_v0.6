...
def read_tf_records(batch_size, tf_records, num_repeats=1,
    ...
    if filter_amount < 1.0:
        dataset = dataset.filter(
            lambda _: tf.random_uniform([], seed=13337) < filter_amount)
...
def get_input_tensors(batch_size, tf_records, num_repeats=1,
    ...
    dataset = dataset.map(
        functools.partial(batch_parse_tf_example, batch_size), num_parallel_calls=4)
    if random_rotation:
        dataset = dataset.map(_random_rotation_pyfunc, num_parallel_calls=4)
